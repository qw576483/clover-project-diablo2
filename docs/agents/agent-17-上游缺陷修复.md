...
> 通用前置按 `docs/agents/_common.md`（**尤其 §3.5**）；改完必须 `tools/run_all_hosts.ps1` 10 宿主全绿。
...
4. **回归断言**（`.ai-tmp/hosts/flowcheck`）：① 已在 Stage 时再发同区进图 ⇒ `Scene.Load` **调用次数不增加**；
   ② 不同区进图 ⇒ 清场 7 项各执行一次再加载。

**验收**：
- [ ] 离线断言（两条）+ 10 宿主全绿
- [ ] Play 实测：进 Stage → **再发一次同区进图** → 无场景重载、无悬空、`console_status` 的 error = 0
      （`client/_dev/p_a16_cycle.cs` 里有现成的"注入重载"路径，可改造复用）
...
4. **回归断言**（`.ai-tmp/hosts/mapcheck` 或 `.ai-tmp/hosts/fullcheck`）：城镇生成后 `NpcPoints.Count == 5`、
...
- 驱动编辑器前先跑 `client/_dev/p_runbg.cs`（失焦不 tick）；截图前等 ≥6 秒。
