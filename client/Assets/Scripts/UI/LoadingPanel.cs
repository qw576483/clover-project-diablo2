...
//
//   本屏的元素 = 原版 `LoadingScreen.cs` 34-70 行的全部元素：**黑底 + 一张 256×256 读条图**；
//     不画进度条 / 百分比文字 / 提示文字 —— 多画一项 = 与原版不一致。
//     进度反馈由**帧号**表达（原版就是这么表达的），驱动源仍是**真实进度**。
//
//   驱动方式：**进度不只来自 `Game.Scene.Load`**。
...
namespace Diablo2.UI
{
    ...
    public class LoadingPanel : UIPanel
    {
        ...
        /// <summary>取原版 10 帧。素材是**一帧一个 PNG**（不是一张条带切 N 帧）⇒ 逐帧按名加载即可。</summary>
