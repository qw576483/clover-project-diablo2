...
//        · 只揭示「本格 + 8 邻域」的做法会让走一大圈后地图仍只有一串 3×3 小块
//          （实测 `explored=9 / opaquePixels=135`）⇒ 本工程不采用。
//        · 现行口径（`Reveal` 纯函数，离线可断言）：**访问过即记忆**（集合只增不减），
...
//     · **已接线**（`Events.MapExplored` 到位后）：Map 每次"某格**首次**被记为已探索"
