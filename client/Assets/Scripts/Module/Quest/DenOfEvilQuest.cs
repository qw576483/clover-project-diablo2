...
// ⚠️ 任务名与目标文本**已换成原版串表（TBL）里的原句**（2026「任务日志 = 原版串」轮），
//    串 id / 键名逐条写在下面常量的注释里；**画面中文 0 条自写**
//    （原版中文是**繁体**，所以这里的字面量就是繁体 —— 这是对的，片 3 已钉死）。
...
namespace Diablo2.Module.Quest
{
    ...
    internal sealed class DenOfEvilQuest
    {
        ...
        /// agent-52 的实机缺陷是"目标行写「剩余 0」、同屏进度行写「剩余怪物：7」"——根因是
        /// **两行不同源**（目标行用 `IMonsterModule.CountInArea(DenOfEvil)`，人一出洞就恒 0；
        /// 进度行用 `required − progress`）。本轮把数字**只留在进度行**一处
        ...
        /// 本函数不再用它拼数字。⚠️ 「人在洞里才报数」这条语义现在由**进度行的数字**表达
