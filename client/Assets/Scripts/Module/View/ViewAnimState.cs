...
//   **在宿主里一行都跑不到**，"玩家受击动作只出 1 帧""攻击播完没回 idle"这类缺陷
//   只能靠人读代码发现（片 W5 的动画审计里，这两条正是靠人读 + 源码行号判出来的）。
//   摘成纯函数之后，宿主可以喂**任意状态组合**、拿真 `SpriteAnimator` 逐帧推进并断言。
//
// 优先级（与原版一致；`docs/agents/agent-07-*.md` §4 的"死亡 &gt; 受击 &gt; 出手 &gt; 移动 &gt; 站立"）：
...
namespace Diablo2.Module.View
{
    ...
    internal static class ViewAnimState
    {
        ...
        /// <para>★ 片 W5：这一项是"玩家受击动作能看见"的关键 —— 旧实现里 `TickPlayer` 的
