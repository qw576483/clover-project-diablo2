...
namespace Diablo2.Module.View
{
    ...
    internal static class EntityHighlight
    {
        ...
        /// <para> 收口（`select` §9-② 的静默失效风险）：`Shader.Find("Sprite")` **会撞名** ——
        ...
        private static Material SharedMaterial()
        {
            ...
            // 取法**只能**是 `Shader.Find`：项目硬闸门 E1（`uicheck`「`Assets/Scripts` 里直连
            ...
            // 校验必须走 **`Material.HasProperty`**：本工程 Unity 版本的可访问 API 里
        ...
        /// <para>属性块里没写过这两个键时返回 0 —— 所以调用方必须先 `Apply` 过一次
