...
// **帧节奏的唯一口径**（`Application.targetFrameRate` / `QualitySettings.vSyncCount`）。
//
// ★ R1-D（用户投诉「人物移动抖动」的候选①，已定根因）：
//   全工程**从未**设过 `Application.targetFrameRate`（唯一写点在引擎
//   `clover-client-unity-engine/Runtime/Presentation/Quality.cs:179-181`，由 `Game.Quality.SetLevel`
//   触发；而本项目**从不**调 `Game.Quality` ⇒ grep 0 命中）⇒ 帧率只由 `QualitySettings.vSyncCount`
...
namespace Diablo2.Core
{
    ...
    public static class FramePacing
    {
        ...
        /// <para>**任何非预期分支都留痕**：原生 API 抛异常（离线自检宿主属正常现象）或读回值不等于
        /// 目标值 ⇒ 各只报一次 Warn；成功 ⇒ 首次（或"确实被改回去过"时）报一条 Info。</para>
        /// </summary>
        public static void Pin(string reason)
        {
            int beforeFps, beforeVsync, fps, vsync;
            try
            {
                beforeFps = Application.targetFrameRate;
                beforeVsync = QualitySettings.vSyncCount;
            ...
            catch (Exception e)
            {
                ...
                Game.Logger?.Warn(Tag,
                    $"[R1-D] 帧节奏设置失败（{e.GetType().Name}: {e.Message}）⇒ 保持引擎默认帧节奏" +
        ...
        {
            return $"[R1-D] 帧节奏已钉死：targetFrameRate={TargetFrameRate} vSyncCount={VSyncCount}" +
                   $"（原因={reason}；改前 targetFrameRate={beforeFps} vSyncCount={beforeVsync}）" +
                   "—— 口径 = 帧率上限恒定、与画质档位无关（旧口径：LOW/MED ⇒ vSync=0 不封顶 ⇒ dt 抖动、HIGH ⇒ vSync=1）。" +
