...
| 基准 | 内容 | 出处 |
|---|---|---|
| **原版素材本身** | 从 `d2data.mpq` / `d2exp.mpq` / CASC 解出的瓦片·物件·角色·怪物·UI·字体·音效，已落位 `client/Assets/Resources/Clover/D2/**`、`Sound/**` | `策划/素材调研.md`、`client/_dev/assetreport.txt` |
...
| 6 | 攻击节奏与命中反馈三件套 | 挥击动画 + 音效 + 受击闪白/飘字 | 同（`.ai-tmp/hosts/combatcheck` 覆盖全链） | 一致（**注**：实机截图本轮未拍，见 `验收表.md` 第 19~21 行的证据口径） |
...
| 13 | 任务链（邪恶洞穴）| 接取 → 找洞 → 清光 → 交任务 → 奖励技能点 +1 | 状态机与目标计数已实现（`.ai-tmp/hosts/flowcheck` 全通过），但**实机整链未走通取证** | 不一致（差在哪：**缺"整链走一遍"的实机证据**，见 `验收表.md` 第 38 行） |
...
| ~~自动地图多了标题/图例文字（原版无）~~ ⇒ **片 5 已消除**（标题/图例整段删除） | — | 验收表第 11 行 + `.ai-tmp/hosts/uicheck` 的 `P5Check`（源码级 0 命中断言） |
...
| ~~多了方向键备选输入~~ ⇒ **片 8 已消除**（连同 `Module/Input` 的读取器 / `Cfg` 开关 / `Game.Setting` 键 / 选项面板那一行） | 可操作 | `验收表.md` **U-1** + bug 表 **B35** + `.ai-tmp/hosts/uicheck` 的 `CheckNoDirectionKeyMove`（源码级 0 命中断言） |
