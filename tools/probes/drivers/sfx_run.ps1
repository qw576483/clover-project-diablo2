# =============================================================================
# sfx_run.ps1 -- ONE Play session for the SFX runtime verdict (the 24 landed
#               original .wav files really load and really PLAY), and the freeze
#               of its evidence.
#
#   powershell -NoProfile -ExecutionPolicy Bypass -File sfx_run.ps1 [-Tag sfx1] [-Save g66]
#
# Chain: editor_focus -> editor_stop -> recompile (+ poll recompile_status)
#        -> clear_console -> editor_play -> Sfx.Api.Cfg -> install
#        tools/probes/drivers/sfx_drive.cs -> the driver boots to a Stage, drives
#        the six triggers the task names, watches the real AudioSource instances
#        and writes a done marker -> wait for it -> echo the [SFX] lines
#        -> console_status / console errors -> editor_stop -> freeze
#        .ai-tmp/screenshots/sfx_evidence_<Tag>.txt
#
# WHY THIS EXISTS: client/资源欠缺清单.md #6 (SFX) was believed to be an empty
# stub; it is implemented and the 24 original .wav files are on disk.  The
# offline host (tools/probes/hosts/audiocheck) proves the BYTES (sha256 vs
# d2sfx.mpq); this driver proves the RUNTIME half: Resources load + AudioSource
# playback + clip.name readback.  Only these rows are re-captured -- the shared
# x/w2 tours are NOT re-run (that would void their rows).
#
# CRITERION CLASS: numeric (runtime log lines + assertions) => no contact sheet.
# The driver still writes a few sfx_00..sfx_05 context screenshots.
#
...
param(
    [string]$Tag = 'sfx1',
    [string]$Save = 'g66',
    [string]$Why = 'SFX runtime verdict: the 24 landed original .wav files must really load from Resources and really play (engine AudioSource.clip.name readback) for the six triggers the task names - swing/hit, being hurt, pickup, UI click, skill cast, monster voice. A playback failure (broken import, missing clip, silent fallback) is invisible to any offline source scan or sha256 check, so only a live Play session can judge it.'
)
...
$cs      = Join-Path $root 'tools\probes\drivers\sfx_drive.cs'
$test    = Join-Path $root '.ai-tmp\test'
$shots   = Join-Path $root '.ai-tmp\screenshots'
$done    = Join-Path $test ('sfx_done_' + $Tag + '.txt')
$trace   = Join-Path $test ('sfx_steps_' + $Tag + '.txt')
$frozen  = Join-Path $shots ('sfx_evidence_' + $Tag + '.txt')
...
Set-Content -Path $trace -Value ('# sfx steps tag=' + $Tag) -Encoding UTF8
...
Add-Content -Path $playLog -Value ((Get-Date).ToString('yyyy-MM-dd HH:mm') + "`tclover-impl`tsfx-original-wavs`t" + $shortWhy) -Encoding UTF8
...
$cfgOk = $false
for ($i = 0; $i -lt 10; $i++) {
    $r = Run-Step 'SfxDrive.Api.Cfg' ''
    if ($r -match 'CFG-OK') { $cfgOk = $true; break }
    Start-Sleep -Seconds 3
}
Say ('CFG-OK ' + $cfgOk)
Run-Step 'SfxDrive.Api.Ping' '' | Out-Null

$spec = $Tag + '|' + $Save + '|' + ($done -replace '\\', '/') + '|' + ($shots -replace '\\', '/')
$raw = Unity-Cmd @('run_script', '--file', $cs, '--entry', 'SfxDrive.Tour.Install', '--args', ('[\"' + $spec + '\"]'))
...
while (((Get-Date) - $t1).TotalSeconds -lt 30) {
    Read-New
    if ((Lines-With '\[SFX\] VERDICT').Count -gt 0) { break }
    Start-Sleep -Milliseconds 500
}
Read-New
Say ('TAIL-SETTLED verdictLines=' + (@(Lines-With '\[SFX\] VERDICT').Count) + ' logLines=' + $script:lines.Count)

foreach ($l in @(Lines-With '\[SFX\] ')) { Say ('EVIDENCE ' + $l) }
...
$deviceLine = (@(Lines-With '\[SFX\] STEP env device=') | Select-Object -First 1)
if (-not $deviceLine) { $deviceLine = '(no [SFX] env line found)' }
...
[void]$hdr.Add('# SFX runtime evidence -- frozen from client/Logs/Editor.log (numeric class: runtime log + assertions)')
[void]$hdr.Add('# batch: sfx-original-wavs   session: ' + $sessionStart.ToString('yyyy-MM-dd HH:mm:ss') + '..' + $sessionEnd.ToString('HH:mm:ss') + '   driver: tools/probes/drivers/sfx_drive.cs')
[void]$hdr.Add('# run: powershell -NoProfile -ExecutionPolicy Bypass -File tools/probes/drivers/sfx_run.ps1 -Tag ' + $Tag)
[void]$hdr.Add('# why: judge whether the 24 landed original .wav files really LOAD and really PLAY at runtime')
[void]$hdr.Add('#      (engine AudioSource.clip.name readback) -- invisible to any offline scan.')
...
[void]$hdr.Add('# ---------------------------------------------------------------------------')
foreach ($l in @(Lines-With '\[SFX\]')) { [void]$hdr.Add($l) }
[void]$hdr.Add('# ---------------------------------------------------------------------------')
[void]$hdr.Add('# RE-RUN: powershell -NoProfile -ExecutionPolicy Bypass -File tools/probes/drivers/sfx_run.ps1 -Tag ' + $Tag)
