# =============================================================================
# fontscale_run.ps1 -- ONE Play session for the SKILL "text too small" verdict
#   (defect family: `D2Label.Create` call sites that omitted fontSize).
#
#   powershell -NoProfile -ExecutionPolicy Bypass -File fontscale_run.ps1 [-Tag fs1] [-Why "..."]
#
# Chain: editor_status (wait for non-compiling) -> play.lock -> editor_stop ->
#   clear_console -> editor_play -> Byline.Api.Cfg (team protocol) -> FS.Api.Cfg ->
#   FS.Tour.Install -> wait for the done marker -> read the [FS] lines ->
#   editor_stop -> release the lock -> land the 6 tiles in .ai-tmp/screenshots/.
#
# The deliverable is (a) the per-label numbers (glyphH in CANVAS px + rendered row
# count, written by the driver) and (b) the 6 composited tiles. A number can be
# checked against UiLayoutGame.FontPx16 = 28.8 -> (int)28 and FontPx30 = 54.
#
# ASCII only (PS 5.1 parses a BOM-less non-ASCII .ps1 as ANSI).
# =============================================================================
param(
    [string]$Tag = 'fs1',
    [string]$Why = 'font-scale verdict: the criterion is the RENDERED glyph size of 6 panel label families (orb numbers / cell counts / shop title / character values / level title / nameplate), which only exists as in-engine numbers + composited pixels inside a live Play session'
)
...
$proj    = $root + '\client'
$cs      = $root + '\tools\probes\drivers\fontscale_drive.cs'
$testDir = $root + '\.ai-tmp\test'
$outDir  = $testDir + '\fontscale'
$rawDir  = $testDir + '\fsraw'
$shotDir = $root + '\.ai-tmp\screenshots'
$done    = $outDir + '\fontscale_done_' + $Tag + '.txt'
$trace   = $outDir + '\fontscale_steps_' + $Tag + '.txt'
$logCopy = $outDir + '\fontscale_log_' + $Tag + '.txt'
$logPath = $proj + '\Logs\Editor.log'
$playLog = $testDir + '\play-log.tsv'
$lock    = $testDir + '\play.lock'

$tileNames = @('fontscale_01_hud.png', 'fontscale_02_inventory.png', 'fontscale_03_shop.png',
               'fontscale_04_character.png', 'fontscale_05_leveltitle.png', 'fontscale_06_nameplate.png')
...
function Read-New() {
    ...
    try {
        ...
        $keep = @($txt -split "`r?`n" | Where-Object { $_ -match '\[FS\]|\[BYLINE\]|\[RUNBG\]|\[HB\]|bitmapUnavailable|位图字模整体不可用|原版中文字模' })
...
Set-Content -Path $trace -Value ('# fontscale steps tag=' + $Tag) -Encoding UTF8
Say ('BEGIN tag=' + $Tag)
Say ('PROJECT ' + $proj)
...
for ($i = 0; $i -lt 40; $i++) {
    ...
    # NOTE: the CLI JSON arrives with escaped quotes when piped through Out-String
    # ({\"compiling\":false}) -- match the escaped form, not the bare one.
...
Set-Content -Path $lock -Value ('font-scale ' + (Get-Date).ToString('o')) -Encoding UTF8
$lockHeld = $true
...
Add-Content -Path $playLog -Value ((Get-Date).ToString('yyyy-MM-dd HH:mm') + "`tfont-scale`tfont-scale-verdict`t" + $Why) -Encoding UTF8
...
$r2 = Run-Step 'FS.Api.Cfg' ''
Say ('FS-CFG ' + $r2)
$r3 = Run-Step 'FS.Api.Paths' (($rawDir -replace '\\', '/') + '|' + ($done -replace '\\', '/'))
Say ('FS-PATHS ' + $r3)
$r4 = Run-Step 'FS.Api.Ping' ''
Say ('FS-PING ' + $r4)

$spec = ($rawDir -replace '\\', '/') + '|' + ($done -replace '\\', '/')
$raw = Unity-Cmd @('run_script', '--file', $cs, '--entry', 'FS.Tour.Install', '--args', ('[\"' + $spec + '\"]'))
Say ('INSTALL ' + ($raw -replace "`r?`n", ' '))

$t0 = Get-Date
while (((Get-Date) - $t0).TotalSeconds -lt 320) {
    Read-New
...
foreach ($l in @(Lines-With '\[FS\] T[0-5]-NODE')) { Say ('EVIDENCE ' + $l) }
foreach ($l in @(Lines-With 'STAGE-READY|TILE|FINISH|SUMMARY|STATION-FATAL|SETUP-FAIL')) { Say ('EVID ' + $l) }
