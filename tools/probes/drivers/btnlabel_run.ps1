# =============================================================================
# btnlabel_run.ps1 -- btn-label-fix slice: ONE Play session that re-runs the proven
# S2 tour (s2_drive.cs) with the previously-proven node monitor (uifix4_dump.cs),
...
#
#   powershell -NoProfile -ExecutionPolicy Bypass -File btnlabel_run.ps1 [-Tag btnlabel] [-Save g66]
#
# Produces:
#   .ai-tmp/screenshots/btnlabel_after.png        (waypoint panel, LIVE, tour tile s2_05)
#   .ai-tmp/screenshots/btnlabel_panel_delayed.png (monitor's own later capture)
#   .ai-tmp/test/uifix4_wp_dump.txt               (node dump incl. every glyph RawImage colour)
#   .ai-tmp/screenshots/btnlabel_evidence.txt     (frozen [S2]+[UF4] runtime rows)
#
...
param(
    [string]$Tag = 'btnlabel',
    [string]$Save = 'g66',
    [string]$Why = 'btn-label-fix: the waypoint panel destination label glyphs were rendered with UiLayoutFlow.ButtonText = the original WideButton.prefab #191919 (0.098, ~2.79:1 contrast against the dark slate medium-button plate, WCAG AA body text needs 4.5:1) so the 13px Chinese glyphs read as a black smear; this session proves the new explicit readable label colour (UiArt.TitleColor, 4.70:1) on the live panel node colours + a screenshot'
)
...
$raw     = Join-Path $test 'raw_btnlabel'
$done    = Join-Path $test ('btnlabel_done_' + $Tag + '.txt')
$trace   = Join-Path $test ('btnlabel_steps_' + $Tag + '.txt')
$frozen  = Join-Path $shots 'btnlabel_evidence.txt'
...
Set-Content -Path $trace -Value ('# btnlabel steps tag=' + $Tag) -Encoding UTF8
...
if (Test-Path $raw) {
    $map = @{
        's2_01_inventory_open.png'         = 'btnlabel_tour_inv_open.png'
        's2_05_waypoint_panel.png'         = 'btnlabel_after.png'
        's2_06_waypoint_travel.png'        = 'btnlabel_tour_wp_travel.png'
        's2_07_bloodmoor_on_entry.png'     = 'btnlabel_tour_wp_entry.png'
        's2_08_bloodmoor_after_travel.png' = 'btnlabel_tour_wp_arrived.png'
    }
    foreach ($f in Get-ChildItem $raw -Filter '*.png') {
        $nm = $f.Name
        if ($map.ContainsKey($nm)) { $nm = $map[$nm] } else { $nm = 'btnlabel_' + $f.Name }
        $dest = Join-Path $shots $nm
        Copy-Item $f.FullName $dest -Force
        Say ('TILE-COPIED ' + $f.Name + ' -> ' + $nm)
    }
}
# the monitor's own later capture of the waypoint panel (the one that is NOT mid-fade)
$mon = Join-Path $shots 'uifix4_wp_panel.png'
if (Test-Path $mon) {
    Copy-Item $mon (Join-Path $shots 'btnlabel_panel_delayed.png') -Force
    Say 'TILE-COPIED uifix4_wp_panel.png -> btnlabel_panel_delayed.png'
}

# ---- freeze --------------------------------------------------------------------
$sessionEnd = Get-Date
$hdr = New-Object System.Collections.ArrayList
[void]$hdr.Add('# btn-label-fix evidence -- fresh runtime rows ([S2] tour + [UF4] node monitor), frozen from client/Logs/Editor.log')
...
[void]$hdr.Add('# run: powershell -NoProfile -ExecutionPolicy Bypass -File tools/probes/drivers/btnlabel_run.ps1 -Tag ' + $Tag)
