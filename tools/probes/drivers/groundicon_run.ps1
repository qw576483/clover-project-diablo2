# =============================================================================
# groundicon_run.ps1 -- ground-item-icon slice: ONE Play session that re-runs the proven
# S2 tour (s2_drive.cs) with the previously-proven node monitor (groundicon_dump.cs),
# to prove the waypoint destination-label glyph colour is now READABLE.
#
# REUSE NOTE: this file is a minimal edit of the sibling driver tools/probes/drivers/
# uifix4_run.ps1 (same tour / same monitor / same gates) -- only the lock owner, the
# tag, the artifact names and the tile mapping changed. Do not re-invent the chain.
#
#   powershell -NoProfile -ExecutionPolicy Bypass -File groundicon_run.ps1 [-Tag groundicon] [-Save g66]
#
# Produces:
#   .ai-tmp/screenshots/groundicon_after.png        (LIVE, tour tile s2_04_drag_to_ground)
#   .ai-tmp/screenshots/groundicon_panel_delayed.png (monitor's own later capture)
#   .ai-tmp/test/groundicon_nodes.txt             (per-node dump: sprite/texture/rect/colour)
#   .ai-tmp/screenshots/groundicon_evidence.txt     (frozen [S2]+[GIC] runtime rows)
#
# ASCII only (PS 5.1 parses a BOM-less non-ASCII .ps1 as ANSI).
# =============================================================================
param(
    [string]$Tag = 'groundicon',
    [string]$Save = 'g66',
    [string]$Why = 'ground-item-icon: a dropped item on the ground used to be drawn as ONLY a solid quality-colour quad (ViewModule.CreateGroundItem built a 40x79 placeholder with SpriteFrames.QualityColor, Normal = white, and ApplyFrame early-returned for IsGroundItem so no art was ever loaded); this session proves a live dropped item now renders the ORIGINAL item icon (same art as the backpack, D2/Items/inv*) on its SpriteRenderer, captured at two moments plus the tour tile s2_04_drag_to_ground'
)
...
$dumpCs  = Join-Path $root 'tools\probes\drivers\groundicon_dump.cs'
$test    = Join-Path $root '.ai-tmp\test'
$shots   = Join-Path $root '.ai-tmp\screenshots'
$raw     = Join-Path $test 'raw_groundicon'
$done    = Join-Path $test ('groundicon_done_' + $Tag + '.txt')
$trace   = Join-Path $test ('groundicon_steps_' + $Tag + '.txt')
$frozen  = Join-Path $shots 'groundicon_evidence.txt'
...
function Release-Lock() {
    ...
    if ($body -match '^\s*ground-item-icon\b') {
...
Set-Content -Path $trace -Value ('# groundicon steps tag=' + $Tag) -Encoding UTF8
...
# extra gate (ui-fix4 lesson): a concurrent teammate can clobber the session --
# wait until Game.Res is really up before driving the tour.
$resOk = $false
for ($i = 0; $i -lt 10; $i++) {
    $r = Run-Step $cs 'S2.Api.Ping' ''
    if ($r -match 'gameRunning=1') { $resOk = $true; break }
    Start-Sleep -Seconds 3
}
Say ('PING gameRunning=' + $resOk)

# passive monitor first: it must be in place BEFORE the tour opens any panel
Run-Step $dumpCs 'GIC.Dump.Install' ((Join-Path $root '.ai-tmp') -replace '\\', '/')
...
if (Test-Path $raw) {
    $map = @{
        's2_01_inventory_open.png'         = 'groundicon_tour_inv_open.png'
        's2_04_drag_to_ground.png'         = 'groundicon_after.png'
        's2_06_waypoint_travel.png'        = 'groundicon_tour_wp_travel.png'
        's2_07_bloodmoor_on_entry.png'     = 'groundicon_tour_wp_entry.png'
        's2_08_bloodmoor_after_travel.png' = 'groundicon_tour_wp_arrived.png'
    }
    foreach ($f in Get-ChildItem $raw -Filter '*.png') {
        $nm = $f.Name
        if ($map.ContainsKey($nm)) { $nm = $map[$nm] } else { $nm = 'groundicon_' + $f.Name }
...
foreach ($m in @('groundicon_mon_1.png', 'groundicon_mon_2.png', 'groundicon_mon_3.png')) {
    $p = Join-Path $shots $m
...
[void]$hdr.Add('# ground-item-icon evidence -- fresh runtime rows ([S2] tour + [GIC] node monitor), frozen from client/Logs/Editor.log')
[void]$hdr.Add('# session: ' + $sessionStart.ToString('yyyy-MM-dd HH:mm:ss') + '..' + $sessionEnd.ToString('HH:mm:ss') + '   drivers: tools/probes/drivers/{s2_drive,groundicon_dump}.cs')
[void]$hdr.Add('# run: powershell -NoProfile -ExecutionPolicy Bypass -File tools/probes/drivers/groundicon_run.ps1 -Tag ' + $Tag)
...
[void]$hdr.Add('# ---------------------------------------------------------------------------')
foreach ($l in @(Lines-With '\[GIC\]')) { [void]$hdr.Add($l) }
...
Say ('SUMMARY tag=' + $Tag + ' cfgOk=' + $cfgOk + ' ping=' + $resOk + ' done=' + (Test-Path $done) +
     ' recompile=' + $recompile + ' failed=' + $failed + ' consoleErrors=' + $consoleErrors +
     ' GIC=' + (@(Lines-With '\[GIC\]').Count))
