...
$proj   = $root + '/client'
$cs     = $PSScriptRoot + '/d2tour_evidence.cs'
$test   = $root + '/.ai-tmp/test'
$shots  = $root + '/.ai-tmp/screenshots'
$done   = $test + '/d2tour_done_' + $Tag + '.txt'
$trace  = $test + '/d2tour_steps_' + $Tag + '.txt'
$frozen = $test + '/d2tour_readings_' + $Tag + '.txt'
$outPath= $test + '/d2tour_out_' + $Tag + '.txt'
...
try {
    ...
    Add-Content -Path $playLog -Value ((Get-Date).ToString('yyyy-MM-dd HH:mm') + "`ttour`t" + $Tag + "`tthe full-screen contact sheet requires the real rendered game: 18 UI panels only exist as live uGUI trees, the 3 area vistas plus the NPC nameplate / monster hover / waypoint anchor animation / inventory drag ghost are all runtime render state that no offline host can produce") -Encoding UTF8
    ...
    Say ('SPEC ' + $spec)
    $inst = Unity-Cmd @('run_script', '--file', $cs, '--entry', 'P2.Tour.Install', '--args', ('[\"' + $spec + '\"]'))
