...
param(
    [string]$Tag = 's2',
    [string]$Save = 'g66',
    [string]$Why = 'S2: (a) the item-drop contract layer (MoveInInventoryRequest -> IItemModule.MoveItem -> Inventory.Move) has never been driven end to end in a live session -- the four drop paths (free cell / swap / outside the panel => ground / oversize refused) must be observed on the real inventory; (b) AppWaypoint.Install was never called from AppWiring, so the whole waypoint chain (click the anchor -> panel -> pick a destination -> area switch) was dead code and only a live run can show it now fires.'
)

$ErrorActionPreference = 'Continue'

# this script lives in <root>/.ai-tmp/drivers => two levels up is <root>
$root    = Split-Path (Split-Path $PSScriptRoot -Parent) -Parent
$proj    = Join-Path $root 'client'
$cs      = Join-Path $root '.ai-tmp\drivers\s2_drive.cs'
$test    = Join-Path $root '.ai-tmp\test'
$shots   = Join-Path $root '.ai-tmp\screenshots'
$raw     = Join-Path $test 'raw_s2'
$done    = Join-Path $test ('s2_done_' + $Tag + '.txt')
$trace   = Join-Path $test ('s2_steps_' + $Tag + '.txt')
$frozen  = Join-Path $shots 's2_evidence.txt'
...
Set-Content -Path $lock -Value ('S2 ' + (Get-Date).ToString('o')) -Encoding UTF8
Say ('LOCK-TAKEN ' + $lock)
...
Add-Content -Path $playLog -Value ((Get-Date).ToString('yyyy-MM-dd HH:mm') + "`tS2`tS2-waypoint+itemdrop`t" + $Why) -Encoding UTF8
...
if (Test-Path $raw) {
    foreach ($f in Get-ChildItem $raw -Filter '*.png') {
        $dest = Join-Path $shots $f.Name
        Copy-Item $f.FullName $dest -Force
        Say ('TILE-COPIED ' + $f.Name + ' bytes=' + $f.Length)
    }
}
