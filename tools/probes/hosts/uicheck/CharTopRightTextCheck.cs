// ─────────────────────────────────────────────────────────────────────────────
// uicheck · 人物属性面板「等级 / 经验 / 技能点」三框的**单行判据**（片 u52-resist，2026-09-24）
//   任务来源：team-lead 派活 **P-2b**（`popupaudit` 的更正清单）。
//
// ⚠️ 先更正任务描述里的形状：原描述"右上框一行拼三段（270×46.8 画布px）"是**旧稿**。
...
//   旧根因（同文件 `:150-151` 注释：三段拼成一行 ≈400 画布px > 凹槽 210 ⇒ 越框 + 越出面板右缘）
//   已由拆框消除 ⇒ 本判据判的是**当前三框各自**放不放得下（这才是现在的缺陷面）。
//
// ── 判据口径（与 ⑧ 同源，⛔ 不另立一套）─────────────────────────────────────
