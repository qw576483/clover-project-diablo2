...
//
// 同一套断言跑**两遍**：`OLD`（修复前，`shim/IsoLayoutOld.cs` 逐字复制）与
// `NEW`（引擎真实源码 `Runtime/Core/IsoLayout.cs`）⇒ 满足「修复前失败 / 修复后通过」。
//
// 判据（三层，互相独立）：
...
//      ⚠️ 单独看 ③ **不足以**发现老缺陷：旧表与旧逆表**互相**自洽（只改一边才会破 ③）
//         ⇒ 必须与 ①②（对着 `GridToWorld` 的几何）一起看。这条差异本身就是证据。
//
// 出处：`GridToWorld` 是 `x = (gx − gy)·HalfW`、`y = −(gx + gy + 1)·HalfH`
//   ⇒ `Δworld = ((Δgx − Δgy)·HalfW, −(Δgx + Δgy)·HalfH)`（W 轴 = 屏幕右，Y 轴 = 屏幕上）。
//   ⚠️ 屏幕上的等距投影把 map 空间的 45° 斜向压成 2:1（斜向 `Δworld` 只有 26.57°）
