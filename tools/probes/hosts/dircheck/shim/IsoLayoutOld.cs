// ─────────────────────────────────────────────────────────────────────────────
// E-core-19 的**修复前对照**（只给最小复现用，跑完即删）
//
// ⚠️ 本文件是 `Runtime/Core/IsoLayout.cs` **改动前**那四个成员的**逐字复制**
//   （唯一改动：类名 `IsoLayout` → `IsoLayoutOld`，因为 `IsoLayout` 是 `sealed` 不能继承）。
//   为什么需要它：`patterns/engine-fix.md` 要求「**修复前失败 / 修复后通过**」的对照，
//   而 `IsoLayout.cs` 在引擎仓库里**尚未提交**（`git status` 显示 `??`）⇒ 拿不到旧版本，
