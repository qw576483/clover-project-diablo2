# -*- coding: utf-8 -*-
"""x_sheet.py -- contact sheet(s) + index for the X full-system tour.

Usage:
    python tools/probes/measure/x_sheet.py <shots_dir> <frozen_log> <trace> <out_dir> [tag]

What it does (judgement belongs to a script, not to the model):
  * reads the FROZEN play log (the `[X] ...` lines the driver printed);
...
values only; the adjudication belongs to the reader.  `x_sheet_selftest.py` re-checks that.
"""
import os
import re
import sys
...
GRID_RX = re.compile(r"\[X\]\s+GRID=(\S+)\s+tile=(\S+)\s+vals=\"(.*)\"\s*$")
SHOT_RX = re.compile(r"\[X\]\s+SHOT-OK\s+n=(\d+)\s+state=\S+\s+name=(\S+)\s+kind=(\S+)\s+bytes=(\d+)")
CROP_RX = re.compile(r"\[X\]\s+CROP\s+n=(\d+)\s+tile=(\S+)\s+node=(\S+)\s+sx0=(-?[\d.]+)\s+sy0=(-?[\d.]+)\s+sx1=(-?[\d.]+)\s+sy1=(-?[\d.]+)")

# ---- sheet split: prefix -> sheet name -------------------------------------
FLOW_PREFIXES = ("B", "C5", "G1", "G2", "G3", "H1", "H2", "H3", "X6", "X7")
GAME_PREFIXES = ("C1", "C2", "C3", "C4", "D1", "D2", "D3", "E1", "E2", "E3", "E4",
                 "F1", "F2", "F3", "G4", "X1", "X3", "X5", "X9")

# ---- cells that deserve two columns (bigger pictures) ----------------------
# X6 (HUD) / X5 (inventory+equipment) / X7 (stats panel) are full-screen UI: give them two
# columns so the panel detail survives the contact-sheet downscale (the criteria are pictures).
WIDE_IDS = ("C1", "C2", "C3", "G4", "D1-detour-start", "D1-detour-stop", "D2", "D2b",
            "X6", "X5-before", "X7-stats", "X9-default", "X9-hover")
...
def strip_tag(line):
    """`[ts] [Info] [X] KEY=v` -> `[X] KEY=v` (strip at most 2 leading tags)."""
    if not line:
        return ""
    s = line.strip()
    for _ in range(2):
...
def build_sheet(name, title, cells, shots_dir, crops, trace, out_png):
    ...
    draw.text((8, 6), "X contact sheet [%s] -- %s" % (name, title), fill=(255, 245, 180))
...
def main(argv):
    ...
    tag = argv[5] if len(argv) > 5 else "x"
    lines = load(frozen)
    ...
    build_sheet("flow", "front end + quest/npc/death screens", flow, shots_dir, crops, trace,
                os.path.join(out_dir, "x_contact_flow.png"))
    build_sheet("game", "world framing + movement + combat + items", game, shots_dir, crops, trace,
                os.path.join(out_dir, "x_contact_game.png"))

    out_tsv = os.path.join(out_dir, "x_contact.index.tsv")
