# -*- coding: utf-8 -*-
"""w2_compare.py -- two-run tile comparison for the W2 tour (run1 vs run2).

Usage:
    python tools/probes/measure/w2_compare.py <run1_dir> <run2_dir> <frozen_log_run1> <out_tsv> [frozen_log_run2]
...
For every tile it reports, as NUMBERS only (no verdict words):
  ...
  * for tiles whose panel rectangle was logged by the driver (`[W2] CROP ...`), it crops BOTH runs to
...
It also copies the `[W2] WORLD tag=... seed=...` lines of both frozen logs into extra rows so the
...
WORLD_RX = re.compile(r"\[W2\]\s+WORLD\s+tag=(\S+)[^\n]*?seed=(-?\d+)[^\n]*?map=(\d+x\d+)")
...
def main(argv):
    ...
    if log2:
        ...
        for ln in load(log2):
            m = WORLD_RX.search(ln)
            if m:
                w2[m.group(1)] = (m.group(2), m.group(3))
        for tag in sorted(set(list(w1.keys()) + list(w2.keys()))):
            world.append((tag, w1.get(tag), w2.get(tag)))
