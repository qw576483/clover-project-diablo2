...
def onemag(a):
    """`isolated_hi`：**8 邻域**近似版"孤立高饱和像素占比"（本脚本自有口径，仅供交叉参考）。

    ⚠️ ⛔ 不要把它当成片 `dialog-options` 的尺 A（`probe_med_sel_palette.py`）：
    那把尺是 **4 邻域** + `ch>60` 高饱和 + 邻域平均 `ch<30`（定义不同、数值不可互比，
    同图实测 0.054(尺A) vs 26.56(尺B=本文件的 speckle)，差两个数量级）。
    """
